...
def test_admit_patient(client, data_entry_token, db_session, test_hospital):
    ...
    response = client.post(f"/api/v1/beds/admit?bed_id={bed.id}&patient_id={patient_id}", headers=headers)
    assert response.status_code == 200
...
def test_discharge_patient(client, data_entry_token, db_session, test_hospital):
    ...
    response = client.post(f"/api/v1/beds/discharge?patient_id={patient_id}", headers=headers)
    assert response.status_code == 200
